...
void loop() {
  ...
  if (radio.available()){
    ...
    if(stringMessage == "TURNON"){

      Serial.println("Looks like they want to turn on the relay!");
    ...
    } else if(stringMessage == "TURNOFF"){

      Serial.println("Looks like they want to turn off the relay!");
      digitalWrite(RELAY_PIN, HIGH);
      const char text[] = "SUCCESS";
      radio.write(text, sizeof(text));
      Serial.println("We sent our message.");

    } else if(stringMessage == "GETREADING"){
      
      Serial.println("Looks like they want a moisture reading!");
      moist = analogRead(A0);
      const char text[32];
      sprintf(text, "SUCCESS.%d", moist);
      radio.write(text, sizeof(text));
      Serial.println("We sent our reading.");
